...
namespace cpl
{
	namespace rendering
	{
		...
		template<typename PixelType, LCDMatrixOrientation orientation>
			using Renderer = CSubpixelScanlineRenderer < PixelType, orientation, WeightMap<5, 9>, LinearGammaScale<std::uint8_t> >;
		...
		bool CSubpixelSoftwareGraphics::tryToDrawGlyph(int glyphNumber, const AffineTransform & z)
		{
			...
			if (glyphSpansMultipleMonitors || !currentMonitor.isApplicableForSubpixels)
				return false;

			// get the glyph outlines
			std::unique_ptr<const juce::EdgeTable, maybe_delete<const juce::EdgeTable>> outlines;
			...
			if (currentMonitor.screenRotation == M_PI)
			{
			...
			gammaScale.setGammaCorrection(colour.getBrightness(), currentMonitor.fontGamma);
				...
				case LCDMatrixOrientation::RGB:
				{
					...
					switch (buffer.getFormat())
					{
						case juce::Image::RGB:
						{
							...
							Renderer<PixelRGB, LCDMatrixOrientation::RGB>
