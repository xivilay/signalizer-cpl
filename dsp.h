	...
	namespace cpl
	{
		...
		template<typename T>
			void fillWithRand(T & vec, size_t size)
			{
				...
				{
					vec[i] = ((double(RAND_MAX) / 2) - rand()) / (double(RAND_MAX) / 2);
				}
		...
		inline void seperateTransformsIPL(double * tsf, std::size_t N)
		{
			...
			for (auto k = 2u; k < N2; k += 2)
			{
				...
				// real[k] = (x1 + x2) * 0.5;
				tsf[k] = (x1 + x2) * 0.5; // real bin N channel 1
				// real[k + 1] = (y1 - y2) * 0.5;
				tsf[N - k] = (y1 - y2) * 0.5; // real bin N channel 2
				// real[k + 1] = (y1 - y2) * 0.5;
				tsf[k + 1] = (y1 + y2) * 0.5; // imag bin N channel 1
				// imag[k + 1] = -(x1 - x2) * 0.5;
				tsf[N - k + 1] = -(x1 - x2) * 0.5; // imag bin N channel 2
			}
