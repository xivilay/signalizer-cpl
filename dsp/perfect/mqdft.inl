...
//#pragma omp parallel for
					for (int idx = 0; idx < numFilters; idx += 8)
					{
						...
						const Vector8 lengths = _mm256_load_ps(unp3);
						const Vector8 wC1 = _mm256_load_ps(unp);
						const Vector8 wC2 = _mm256_load_ps(unp2);

						Vector8 ftSin = _mm256_setzero_ps();
						Vector8 ftCos = _mm256_set1_ps(1.0f);
						Vector8 wSin = _mm256_setzero_ps();
						Vector8 wCos = _mm256_set1_ps(1.0f);


						Vector8 imag = _mm256_setzero_ps(), real = _mm256_setzero_ps();
						Vector8 t0, t1, ones = _mm256_set1_ps(1.0f);
						...
						for (; t < minLength; t++)
						{
							...
							Vector8 input = _mm256_broadcast_ss(dataptr + t);
...
#define _DOTAIL
						...
						for (; t < maxLength; ++t)
						{
							Vector8 loopcount = _mm256_set1_ps(t);
							...
							auto mask = _mm256_cmp_ps(loopcount, lengths, _CMP_LE_OQ);

							Vector8 input = _mm256_broadcast_ss(dataptr + t);
							input = _mm256_and_ps(input, mask);
							// window function
							t0 = _mm256_sub_ps(ones, wCos);
						...
						// scale transform
						auto scaledLengths = _mm256_mul_ps(lengths, _mm256_set1_ps(0.5f)); // 0.5 cos hanning window without * 2

						real = _mm256_div_ps(real, scaledLengths);
						imag = _mm256_div_ps(imag, scaledLengths);
						// change sign on imaginary part
						imag = _mm256_mul_ps(imag, _mm256_set1_ps(-1.0f));
						// store accumulated filter values
