...
//#pragma omp parallel for
					for (int idx = 0; idx < numFilters; idx += 8)
					{
						...
						const Vector8 ftC1 = _mm256_load_ps(unp);
						const Vector8 ftC2 = _mm256_load_ps(unp2);
						Vector8 ftSin = _mm256_setzero_ps();
						Vector8 ftCos = _mm256_set1_ps(1.0f);
						...
						Vector4 phases = _mm_setr_ps(0, wOmega, wOmega * 2, wOmega * 3);
						cpl::sse::sincos_ps(phases, &t14, &t24);

						wCos = _mm256_castps128_ps256(t24);
						wSin = _mm256_castps128_ps256(t14);

						phases = _mm_setr_ps(wOmega * 4, wOmega * 5, wOmega * 6, wOmega * 7);
						cpl::sse::sincos_ps(phases, &t14, &t24);

						wCos = _mm256_insertf128_ps(wCos, t24, 1);
						wSin = _mm256_insertf128_ps(wSin, t14, 1);
						/*fast_math::sincosf(wOmega, &wSin.ref_g(), &wCos.ref_g());
						fast_math::sincosf(wOmega * 2, &wSin.ref_b(), &wCos.ref_b());
						fast_math::sincosf(wOmega * 3, &wSin.ref_a(), &wCos.ref_a());*/

						Vector8 imag = _mm256_setzero_ps(), real = _mm256_setzero_ps();
						Vector8 t0, t1, t2, t3, t4, ones = _mm256_set1_ps(1.0f);
						...
						int t = start;
						unsigned remainder = ftLength & 0x7;
						for (; t < length; t += 8)
						{
...
#define FourierTransform256(input, fcos, fsin, real, imag) \
	...
	imag = _mm256_add_ps(imag, _mm256_mul_ps(input, fsin));
							Vector8 rawinput = _mm256_load_ps(dataptr + t);
							...
							phases = _mm_setr_ps(t * wOmega + (wOmega * 0), t * wOmega + (wOmega * 1), t * wOmega + (wOmega * 2), t * wOmega + (wOmega * 3));
							t14 = cpl::sse::cos_ps(phases);

							wCos = _mm256_castps128_ps256(t24);
							phases = _mm_setr_ps(t * wOmega + (wOmega * 4), t * wOmega + (wOmega * 5), t * wOmega + (wOmega * 6), t * wOmega + (wOmega * 7));
							t14 = cpl::sse::cos_ps(phases);

							wCos = _mm256_insertf128_ps(wCos, t14, 1);
							// window function
							t0 = _mm256_sub_ps(ones, wCos);
							...
							rawinput = _mm256_mul_ps(t0, rawinput);

							// same for the window
							//RotateFilterA256(wCos, wSin, wC1, wC2, t0);


#endif

							Vector8 input = _mm256_broadcastidx_ps(rawinput, 0);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 1);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 2);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 3);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 4);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 5);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 6);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);

							input = _mm256_broadcastidx_ps(rawinput, 7);
							FourierTransform256(input, ftCos, ftSin, real, imag);
							RotateFilterA256(ftCos, ftSin, ftC1, ftC2, t0);
						...
						/*	use this bit of code to enable non-quantized lengths,
							when dft finally gets ability to run reversed
						// last iteration of window
						RotateFilterA256(wCos, wSin, wC1, wC2, t0);
						// store
						_mm256_store_ps(unp, wCos);


						for (int i = (signed)length - remainder; i < length; ++i)
						{
							auto f = dataptr[i] * (1 - unp[i]);

							Vector8 input = _mm256_set1_ps(f);
						...
						_mm256_store_ps(unp, real);
						_mm256_store_ps(unp2, imag);
						for (int i = 0; i < 8; ++i)
						{
							result[idx * 2 + i * 2] = unp[i] / (ftLength / 2);

							result[idx * 2 + 1 + i * 2] = -unp2[i] / (ftLength / 2);
						}
