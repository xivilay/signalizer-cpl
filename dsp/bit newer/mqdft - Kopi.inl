...
#define RotateFilterA256(fcos, fsin, c1, c2, temp) \
	...
	fcos = temp; 
							...
							t3 = _mm256_mul_ps(ftCos, ftC2); // c2 * cos
							t4 = _mm256_mul_ps(ftSin, ftC1); // c1 * sin

							Vector8 input = _mm256_broadcastidx_ps(rawinput, 0);
							// fourier transform
							t0 = _mm256_mul_ps(input, ftCos);
							real = _mm256_add_ps(real, t0);
							ftCos = _mm256_mul_ps(ftCos, ftC1);
							t1 = _mm256_mul_ps(input, ftSin);
							imag = _mm256_add_ps(imag, t1);
							ftSin = _mm256_mul_ps(ftSin, ftC2);

							
	
							ftCos = _mm256_sub_ps(ftCos, ftSin); // final cosine, t0
							// these two lines have no dependency on the upper three, so should be pipelined


							ftSin = _mm256_add_ps(t3, t4); // final sine, t1
